...
def decrypt(string, number):
	result =""
	for ltr in string:
		if(alphabet.find(ltr)<0):
			result+=" "
		else: result += alphabet[(alphabet.find(ltr)+number)%len(alphabet)]
	return result
...
for input in inputArray:
	letters, digits, valid = rowInfo(input)
	if valid: 
		print(decrypt(letters,digits), digits)
